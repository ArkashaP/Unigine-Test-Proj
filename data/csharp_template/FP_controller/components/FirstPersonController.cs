...
#pragma warning restore CS0649
	...
	private Vec3 slopeAxisX = Vec3.RIGHT;
	private Vec3 slopeAxisY = Vec3.FORWARD;

	public Vec3 SlopeNormal { get { return slopeNormal; } }
	public Vec3 SlopeAxisX { get { return slopeAxisX; } }
	public Vec3 SlopeAxisY { get { return slopeAxisY; } }
		...
		if (useObjectsInteraction)
		{
			...
			for (int i = 0; i < contactsCount; i++)
			{
				...
				if (c.Object && c.Object.BodyRigid)
				{
					...
					c.Object.BodyRigid.AddWorldImpulse(c.Point, (vec3)(-c.Normal * c.Object.BodyRigid.Mass * speed * impulseMultiplier * 0.5f));
				}
			}
	...
	private void UpdateVelocity(float ifps, float updatePart)
	{
		...
		{
			slopeAxisX = MathLib.Cross(new Vec3(worldTransform.AxisY), slopeNormal);
			if (slopeAxisX.Length2 > 0)
				slopeAxisX.Normalize();
			slopeAxisY = MathLib.Cross(slopeNormal, slopeAxisX);
			if (slopeAxisY.Length2 > 0)
				slopeAxisY.Normalize();
		}
		else
		{
			slopeAxisY = MathLib.Cross(new Vec3(worldTransform.AxisX) * MathLib.Sign(cosAngle), slopeNormal);
			if (slopeAxisY.Length2 > 0)
				slopeAxisY.Normalize();
			slopeAxisX = MathLib.Cross(slopeAxisY, slopeNormal);
			if (slopeAxisX.Length2 > 0)
				slopeAxisX.Normalize();
		}
	...
	private void MoveSide(float ifps)
	{
		...
		for (int i = 0; i < contactsCount; i++)
		{
			...
			if (MathLib.Dot(c.Point - shape.BottomCap, Vec3.UP) < 0.0f)
			{
				...
				if (!MathLib.Equals(cosAngle, 0.0f, 0.01f) && cosAngle < maxCosAngle)
				{
	...
	private void TryMoveDown(float ifps)
	{
		...
		float inumContacts = 1.0f / contactsCount;
		for (int i = 0; i < contactsCount; i++)
		{
			ShapeContact c = contacts[i];
	...
	private void UpdateCrouch(float ifps)
	{
		...
		if (crouchState.currentTime > 0.0f)
		{
			...
			float t = 1.0f;
			if (!MathLib.Equals(crouchTransitionTime, MathLib.EPSILON))
